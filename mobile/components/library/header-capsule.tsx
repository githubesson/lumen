...
import { DOCK_COLLAPSE_SPRING, useDockColors } from "../dock/dock-context";
 ...
 * plain action (home).
 *
 * The open/close morph runs on the dock's own motion: a shared value
 * driven by DOCK_COLLAPSE_SPRING (the same near-critically-damped spring
 * as the dock's expand/collapse) interpolated on the UI thread — width and
 * cross-fades all derive from the one value, and reduced motion snaps
 * instead of animating. The wrapper nudges the capsule toward the screen
 * edge via margin, never a transform: the interactive Liquid Glass view
 * computes its frame without ancestor transforms, so a transformed wrapper
 * leaves the glass offset from the scrim/border layers (the double-capsule
 * ghost).
 *
