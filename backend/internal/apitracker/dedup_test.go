...
func TestPreviousStillPresentAfterDedup(t *testing.T) {
	...
	defer pool.Close()
	store := NewStore(pool)
	pin, err := store.AddPin(ctx, AddPinInput{APIBaseURL: "https://tracker.example/api", TrackerID: 1, RootPath: t.TempDir()})
	if err != nil {
		t.Fatal(err)
	}
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			for i := 0; i < 2; i++ {
				summary := ScanSummary{}
				if got := scanner.previousStillPresent(ctx, pin.ID, sourceURL, TrackContext{Title: "Wrong title"}, "", &summary); got != tc.want {
